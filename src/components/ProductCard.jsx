...
function ProductCard(props) {
  ...
  return (
    <div
      className={`w-[290px] h-[345px] rounded-2xl shadow-black shadow-3xl cursor-pointer hover:shadow-4xl transition-all duration-300`}
    >
      <div className="cardImageDiv pt-4 px-4">
        ...
        <img
          className="h-[150px] w-[120px] py-4"
          src={image}
          alt="mens-cloth-img"
        />
      </div>
      <div className={`py-2 ${color} rounded-2xl h-[90px] px-6 mt-7`}>
        <h2 className="text-center text-blue-600 text-2xl font-bold pb-2">
          {price}
        </h2>
        <p className="text-center font-medium truncate">{description}</p>
      </div>
    </div>
  );
}
